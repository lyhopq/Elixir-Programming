defmodule Libfind do
  ...
  def find_same_picture(dir) do
    files(dir, ~r/.*\.jpg/, false,
      fn(file, acc) ->
        md5 = MD5.cks(file)
        if Dict.has_key?(acc, md5) do
          Dict.update(acc, md5, [file], fn v -> [file | v] end)
        else
          Dict.put(acc, md5, [file])
        end
      end,
      HashDict.new
    )
    |> Enum.filter_map(fn {_md5, files} -> length(files) > 1 end,
                       fn {_md5, files} -> files end)
  end
end
